...
df_routes = pd.DataFrame(
    [], columns=["orig_code"] + list(dest_columns_and_rename.values())
)

for origin_fold in os.listdir(path=routes_data_loc):
    destinations_files = [
        i for i in os.listdir(path=routes_data_loc + f"\\{origin_fold}") if "to_" in i
    ...
    for dest in destinations_files:
        ...
        df_dest = df_dest.loc[:, dest_columns_and_rename.keys()].rename(
            columns=dest_columns_and_rename
        )
...
df_routes = df_routes.merge(
    city_data, how="left", left_on="dest_city_code", right_on="Id"
).drop(["Id", "CityName", "CountryName", "CountyCode", "ImageUrl"], axis=1)
...
for origin in origins_data.keys():
    orig_df = df_routes[
        (df_routes.orig_code == origin) & (df_routes.dest_price_dir <= price_cap)
    ]
    ...
    for i in orig_df.itertuples():
        ...
        fig.add_trace(
            go.Scattermapbox(  # MARKERS
                ...
                text=[
                    i.dest_city
                    + f", {i.dest_country_name} from €{int(i.dest_price_dir)}"
                ],
                hoverinfo="text",
                mode="markers",
                legendgroup=origin,
...
#     ),
# )
webbrowser.get(r"C:/Program Files (x86)/Google/Chrome/Application/chrome.exe %s").open(
    r"D:\Code\Others\FlightsScraper\hyperlink_fig2.html"
)
