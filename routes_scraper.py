...
sleep_time = 5 #in seconds so we will randomly get up to 3 seconds sleep
...
outbound_yr_m = '2021-12'
inbound_yr_m = '2021-12'
...
for origin in origins:
    ...
    for country in dest_countries_prices[origin].keys():
        ...
        if allow_requests: #Cache and check
            ...
            print(round(sleep,1),f' seconds currently at origin and destination: {origin}, {country} ; {c_out_of} out of {out_of}')
            time.sleep(sleep)   
        ...
        df_cit['IndirectPrice'] = np.NaN if 'IndirectPrice' not in df_cit.columns else df_cit['IndirectPrice']
...
if False:
    ...
    for date_fold in date_locs:
        ...
        for origin_fold in os.listdir(path = routes_data_loc + f'\\{date_fold}'):
            dests = [i for i in os.listdir(path = routes_data_loc + f'\\{date_fold}' +
                                           f'\\{origin_fold}') if len(i) > 11] # check if origin + dest file
            for dest in dests:
                df_dest = pd.read_csv(routes_data_loc + f'\\{date_fold}' +f'\\{origin_fold}' + f'\\{dest}')
    ...
    for c,i in enumerate(df_cc.itertuples()):
        ...
        try:
            ...
            mp[i.CityName] = {'country':fcs['country'],
                              'lon':fcs['longitude'],
                              'lat':fcs['latitude']}
        except:
